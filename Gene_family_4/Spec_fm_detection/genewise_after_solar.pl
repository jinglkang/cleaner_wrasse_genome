...
#       $p_length=$_[1];
        ...
        $p_length=length $seq{$p_name}; # get the protein name

        $strand= $_[4] eq '+' ? "tfor" : "trev";
#       $d_start= int($ds-3/2*$p_length) >0? int($ds-3/2*$p_length) : 1;
        # Extend the upstream [2*3*length(pep)] and downstream [2*3*length(pep)]
        $e_length=2*3*$p_length;
        $d_start= int($ds-$e_length) >0? int($ds-$e_length) : 1;
#       $d_end= int($de+3/2*$p_length) <$d_length? int($de+3/2*$p_length) : $d_length;
        $d_end= int($de+$e_length) <$d_length? int($de+$e_length) : $d_length;
        ...
        close D;
        $k++;
        `genewise $fname.p.fa $fname.d.fa -quiet -$strand -pseudo -genesf >WISE.$k`;
        open OUT, ">>$fname.wise";
        open GENE_WISE, "WISE.$k";
        while (<GENE_WISE>) {
                chomp;
