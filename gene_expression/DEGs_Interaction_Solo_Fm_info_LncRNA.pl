...
my $Lnc="gene_DELnc.txt";
my %LNC;
open LNC, $Lnc or die "can not open $Lnc\n";
while (<LNC>) {
        chomp; my @a=split /\t/;
        $LNC{$a[0]}=$a[1]."\t".$a[2];
}

print "Gene\tGene_ano\tDEG_nb\tDEG_Info\tFm_id\tFm_name\tFm_ano\tEp_Ct\tEp_Ct_nb\tPos\tLncRNA_nb\tDE_LncRNA\n";
foreach my $gene (keys %DEGs_nb) {
        ...
        my ($DEGs_FB, $DEGs_HB, $DEGs_MB, $info, $fm, $pos, $Lnc_info);
        ...
        $LNC{$gene}    ?($Lnc_info=$LNC{$gene}):($Lnc_info="-\t-");
        if ($DEGs_HB !~ /-/ || $DEGs_MB !~ /-/ || $DEGs_FB !~ /-/) {
                $info=$DEGs_HB."; ".$DEGs_MB."; ".$DEGs_FB;
        }
        print "$gene\t$gene_ano\t$DEG_nb\t$info\t$fm_id\t$fm_ano\t$fm\t$pos\t$Lnc_info\n";
}
